...
const WithdrawMoney = ({ open, handleClose, shopPk, balance, fetchShopDetail, fetchData }) => {
    ...
    const handleCashChange = (id, value, maxQty) => {
        const qty = value === "" ? "" : Math.min(parseInt(value) || 0, maxQty);
    ...
    // Bank Handler
    const handleBankInputChange = (id, value, maxBal) => {
        const val = value === "" ? "" : Math.min(parseFloat(value) || 0, maxBal);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast.error(error.response?.data?.message || "Withdraw submission error");
        } finally {
            setLoading(false);
        }
    };
                ...
                <Form onSubmit={handleSubmit}>
                    ...
                    <Modal.Footer>
                        ...
                        <Button type="submit" variant="success" disabled={loading}>
                            {loading ? <Spinner animation="border" size="sm" /> : "Withdraw Money"}
                        </Button>
                    </Modal.Footer>
                </Form>
            </Modal>
