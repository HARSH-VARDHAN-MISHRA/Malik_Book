...
const PayPaymentModal = ({ open, handleClose, shopPk }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const hasValidCash = cashDenominations.some(d => Number(d.quantity) > 0);
        const hasValidBank = bankPayments.some(b => Number(b.amount) > 0);
        
        if (!hasValidCash && !hasValidBank) {
    ...
    return (
        <Modal show={open} onHide={!loading ? handleClose : null} centered size="lg">
            <Modal.Header closeButton={!loading}>
                <Modal.Title>
                    Make Payment 
                    <span className="fs-6 text-success ms-1 font-medium">{grandTotal ? `(₹${Number(grandTotal).toFixed(2)}) ` : null}</span>
                </Modal.Title>
            ...
            <Form onSubmit={handleSubmit}>
                <Modal.Body>
                    ...
                    <hr />
                    <h5>
                        Bank Payments <span className="fs-6 text-success">(₹{bankTotal.toFixed(2)})</span>
                    </h5>
                        ...
                        <div key={index} className="row align-items-end">
                            <div className="col-md-6 mb-3">
                                ...
                                <select
                                    ...
                                    onChange={(e) => handleBankChange(index, "bankId", e.target.value)}
                            ...
                            <div className="col-md-4 mb-3">
                                ...
                                <input
                                    ...
                                    onChange={(e) => handleBankChange(index, "amount", e.target.value)}
                                    placeholder="Amount"
                                    className="form-control"
                                />
                            </div>
                            ...
                            <div className="col-md-2 mb-3">
                                <Button
                                    variant="danger"
                                    onClick={() => removeBankPayment(index)}
